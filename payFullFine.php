...
if($bookCount)
{
	$sql = "select sum(fine_amt) as fine from fines where loan_id in (select loan_id from book_loans where card_id = '".$borrowerId."')";
	$result = $con->query($sql);
	$row = $result->fetch_assoc();
	$sum_amt = $row["fine"];
	if($sum_amt){
		...
		echo "<tr>";
		echo "<td>".$sum_amt."</td>";
		echo "<td><button type='button' class='btn btn-success' onclick='payFine()'>Pay Fine</button></td>";
		echo "</tr>";
		echo "</table>";
	}
	else{
